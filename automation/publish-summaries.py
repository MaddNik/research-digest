...
def load_existing_entries(path):
    """Parse the current _data/summaries.yml into raw per-entry blocks.

    Returns a list of {"slug", "date", "raw": [lines]} preserving each entry's
    exact text. Used to keep summaries that were authored directly in the repo
    (e.g. from the cloud editor) and therefore have no research-material source:
    without this they would be dropped from the manifest on every republish.
    Dependency-free and format-specific to what this script itself writes."""
    out = []
    try:
    ...
    for line in raw.split("\n"):
        if line.startswith("- slug:"):
            ...
            cur = {"slug": m.group(1) if m else "", "date": "", "raw": [line]}
...
def main():
    ...
    def entry_lines(e):
        ...
        for k in ("slug", "title", "url", "date", "category", "pub", "publisher", "meta_line", "source_type", "source_url", "pdf"):
            ...
            L.append('%s%s: "%s"' % (prefix, k, yaml_escape(e[k])))
        tag_items = ", ".join('"%s"' % yaml_escape(t) for t in e.get("tags", []))
        L.append('  tags: [%s]' % tag_items)
        return L

    regen_slugs = set(e["slug"] for e in entries)
    items = [(e["date"], entry_lines(e)) for e in entries]

    # Preserve entries that exist on disk under summaries/ but have no
    # research-material source (authored directly in the repo, e.g. from the
    # cloud editor). To fully remove such a summary, delete its summaries/<slug>/
    # folder as well as its manifest row. On any parse error, fall back silently
    # to the research-material-only manifest (never crash the publisher).
    preserved = 0
    try:
        for pe in load_existing_entries(MANIFEST):
            slug = pe["slug"]
            if slug and slug not in regen_slugs and \
               os.path.isfile(os.path.join(DEST, slug, "index.html")):
                items.append((pe["date"], pe["raw"]))
                preserved += 1
    except Exception:
        preserved = 0
    ...
    lines = [
        ...
        "# Regenerated from research-material; entries with a summaries/<slug>/",
        "# folder but no research-material source are preserved as-is.",
        "",
    ]
    for _date, blk in items:
        lines.extend(blk)
    open(MANIFEST, "w", encoding="utf-8").write("\n".join(lines) + "\n")
    print("published %d summaries (%d preserved) -> %s" % (len(entries), preserved, MANIFEST))
